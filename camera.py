...
class Camera:
    def __init__(self, settings, visible_sprites):
    ...
    def draw(self, player):
        ...
        for sprite in self.visible_sprites:
            ...
            #     next_topleft = ((before_topleft[0]/self.settings.tile_size) * next_tile_size,
            #                     (before_topleft[1]/self.settings.tile_size) * next_tile_size)
            ...
            if -2 * next_tile_size <= position[0] <= self.settings.screen_width + 2 * next_tile_size \
                    and -2 * next_tile_size <= position[1] <= self.settings.screen_height + 2 * next_tile_size:
